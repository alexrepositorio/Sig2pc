...
$SQL="SELECT * FROM lotes order by fecha desc";
	...
	switch ($_GET["criterio"])
		...
		case "organico":
			if($_GET["opcion"]=="si"){
			$SQL="SELECT * FROM lotes LEFT JOIN (SELECT id_socio, year, estatus FROM certificacion WHERE year=(select max(year) from certificacion i where i.id_socio = certificacion.id_socio)) t on lotes.id_socio=t.id_socio WHERE t.estatus='O'";
...
/*	
$SQL_pago="SELECT * FROM pagos where lote=".$lotes["codigo_lote"]." order by fecha desc";
$r_p=mysqli_query($link, $SQL_pago);
$cuenta_p=mysqli_num_rows($r_p);
if($cuenta_p==0){$estados[$row["id"]][]="<font color=blue><b>PENDIENTE</b></font>";}else{
$pago = mysqli_fetch_array($r_p,MYSQLI_ASSOC);
	if($pago["costo"]>0){$color="green";}else{$color="red";}
$estados[$row["id"]][] = "<font color=$color>".$pago["movimiento"]."</font>";
//$costos[]=$pago["costo"];
}
*/	
}
